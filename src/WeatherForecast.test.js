...
describe("weather forecast", () => {
    ...
    beforeAll(() => {
        ...
        mock.onGet("http://api.openweathermap.org/data/2.5/forecast/daily").reply(200, serviceResponse);
    });
    ...
    it("should display an error message when load by id fails", done => {
        mock.reset();
        mock.onGet("http://api.openweathermap.org/data/2.5/forecast/daily").replyOnce(500, { message: "Test" });
        const wrapper = shallow(<WeatherForecast modifyCity={modifyCity} />);
        const event = { target: { name: "", value: 360630 } };
        wrapper.find("#citySelect").simulate("change", event);
        setTimeout(() => {
